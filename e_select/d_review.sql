### c_select 폴더 >>> d_review ###

# korea_db 데이터베이스의 members, purchases 테이블을 기반으로 
# , select ~ form ~ where 구문 전체 흐름 복습
use korea_db;
...
where
	member_id = (
        ...
        from purchases
        group by member_id
        order by sum(amount) desc -- 총합이 가장 큰 레코드부터 출력
...
from 
	members 
limit 2
offset 2; -- 첫 행 시작번호가 0(index 번호와 동일)

select 
	* 
from 
	members 
limit 2, 2; -- 'offset 시작번호'는 생략 후 콤마 사용 가능
