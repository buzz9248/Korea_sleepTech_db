...
as
	select 
		B.mem_id 'Member Id', M.mem_name as 'Member Name', B.prod_name 'Product Name', concat(M.phone1, M.phone2) as 'Office Phone'
        # 별칭(alias)는 작은/큰 따옴표 가능
	from
		buy B
			inner join member M
...
as
	select 
		B.mem_id '회원 아이디', M.mem_name as '회원 이름', B.prod_name '제품 이름', concat(M.phone1, M.phone2) as '연락처'
    from
		buy B
			join member M
            on B.mem_id = M.mem_id;
...
# show create view 뷰이름;: 생성된 뷰의 sql코드를 확인
