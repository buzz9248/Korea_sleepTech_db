...
/*
    ...
    4. 자체 조인(Self join): 자기 자신 테이블과 조인, 동일 테이블 비교
*/
...
from
	members M
		inner join purchases P -- inner join은 가장 기본 조인이기 때문에 inner 키워드 생략 가능
        on M.member_id = P.member_id
group by M.name
order by total_purchases desc;

# 내부 조인 예제 4
# : 단 한번이라도 구매한 기록이 있는 회원 목록을 추출
select
