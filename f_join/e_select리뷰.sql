...
# , select ~ from ~ where ~ group by ~ order by ~ limit 구문의 전체 흐름과 처리 순서
# +) join

/*
	지역(area_code)별로 -- group by
    여성 회원들의 -- where
	지역(area_code)별로 여성 회원들의 총 구매 금액(purchases.amount 합계)를 구하되, - sum 집계함수
...
select
	M.area_code '지역', sum(P.amount) '총 구매 금액', count(distinct M.member_id) '회원 수'
from
	members M
